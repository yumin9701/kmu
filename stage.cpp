...
void game() {
    ...
    while (!snake.getEnd()) {
        WINDOW *win1 = newwin(25, 35, 0, 0);
        print_score(scoreBoard, snake.getLen(), snake.getLevel(), snake.growth_item, snake.poison_item, snake.getGateCnt(), num_mission_size);
        print_mission(mission, snake.getLevel());

        srand(time(NULL));
        char *map_table = snake.map_changer(map[snake.getLevel() - 1]);
        nodelay(win1, TRUE);
        keypad(win1, TRUE);
        refresh();
