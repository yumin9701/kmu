    ...
    for (unsigned int i = 0; i < 25; i++) {
        for (int j = 0; j < 35; j++) {
            switch (map[i][j]) {
                ...
                case 98: map_change[i * col + j] = '8'; break;
                case 99: map_change[i * col + j] = '9'; break;
            }
        }
    }
...
void Snake::make_gate(int map[25][35]) {
    ...
    map[gate[0].second][gate[0].first] = 98;  // 게이트[0] 좌표에 게이트 번호 설정
    map[gate[1].second][gate[1].first] = 99;  // 게이트[1] 좌표에 게이트 번호 설정
}
